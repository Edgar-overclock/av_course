...
class TurtleNavigationNode(Node):
    ...
    def odom_callback(self, msg: Odometry):
        ...
        distance_to_goal = math.sqrt(
            ...
            ((current_pose.position.y + 2) - goal_pose['y']) ** 2
        )
        #self.get_logger().info(f"dist to goal : {distance_to_goal}")
        self.get_logger().info(f"X : {current_pose.position.x}, Y : {current_pose.position.y + 2} ")
        if distance_to_goal < 0.3:  # Threshold to consider the goal reached
            self.publish_next_goal()

    def publish_next_goal(self):
        self.get_logger().info("publishing next is active")
        if self.current_goal_index < len(self.goal_poses) - 1:
            self.get_logger().info("publishing next is in if statement")
            self.current_goal_index += 1
            self.publish_goal()
        else:
            self.get_logger().info("publishing next is in else statement")
